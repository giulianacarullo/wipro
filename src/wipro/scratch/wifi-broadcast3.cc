...
int main (int argc, char *argv[]) {
  ...
  //Creating source nodes and setting them to be sink nodes as well
  ...
  wifi.SetStandard (WIFI_PHY_STANDARD_80211b); //check if 802.11a or 802.11b
  ...
  for (int nodeID = 0; nodeID < (int) numNodes; nodeID++){
	  Ptr<Socket> beacon_sink = Socket::CreateSocket(sink_nodes.Get(nodeID),tid);
...
/*
	  ...
	  Ptr<Socket> beacon_source = Socket::CreateSocket(source_nodes.Get(nodeID), tid);
	  beacon_source->Connect(beaconBroadcast);
	  std::cout << "Node "<< nodeID << " sending packet\n";
 ...
 // Tracing
 wifiPhy.EnablePcap ("wifi-experiment-broadcast", sink_devices);
